...
{% macro test_flatten_column() %}
  ...
  {{ dbt_unittest.assert_equals(
    fhir_dbt_utils.spark__flatten_column({"name": "col", "data_type": "boolean"}),
    []) }}
  {{ dbt_unittest.assert_equals(
    fhir_dbt_utils.spark__flatten_column({"name": "col", "data_type": "struct<foo:string>"}), [
      {'name': 'col.foo', 'data_type': 'string'}
    ]) }}
  {{ dbt_unittest.assert_equals(
    fhir_dbt_utils.spark__flatten_column({"name": "col",
