...
{% macro test_select_from_unnest() -%}
  {{ return(adapter.dispatch('test_select_from_unnest', 'fhir_dbt_utils') ()) }}
{%- endmacro %}


{% macro default__test_select_from_unnest() -%}
  {{ fhir_dbt_utils.assert_string_equals(
      fhir_dbt_utils.select_from_unnest("c.code", "UNNEST(my_array) c", where="c.value > 1", order_by="c.key"),
...
{% macro spark__test_select_from_unnest() -%}
  {{ fhir_dbt_utils.assert_string_equals(
      fhir_dbt_utils.spark__select_from_unnest(
        "c.code",
